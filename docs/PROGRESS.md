...
| T070 | UI: mapa-múndi de regiões | `ui/screens/worldMapScreen.ts`: cadeia ramificada **apenas das regiões descobertas**; regiões futuras nunca reveladas antecipadamente (aparecem como `❓` nas fronteiras quando `hasFrontier`). Marca a região atual. Ícones data-driven via `REGION_ICONS` (content). Absorve a T059. 2 testes |
| T071 | Integração no loop + i18n | `game.ts` ganhou o loop **`exploreGraph`** (viajar/mapa/diário/mundo) e um despachante `explore` que escolhe grafo vs **`exploreLinear`** (legado, preservado para `startingFields` e seus testes). `visitLocation`→`resolveLocationContent` resolve combate/boss/npc/loja/lore e marca o local concluído; `runDialogue`/`applyEffects` aplicam efeitos (conhecimento, revelar, missão, loja, combate) com feedback. Novo jogo começa no **Bosque Sombrio**. `GameIO` ganhou `travel`/`talk`/`regionMap`/`journal`/`worldMap` (opcionais). `index.ts` implementa os 5 no inquirer e ramifica o menu por `isGraph`. i18n completo en/ptBR (locais, conhecimentos, diálogos, missão, mapa/diário/mundo, mensagens do loop, erros). `game.test.ts` reescrito para o fluxo em grafo (playthrough até o Necromante com cura, derrota do mago, quest do caçador, revelação por lore, fuga) + menu/idioma. Testes de UI das T067–T070 ajustados para `t()`. 328 testes |
| T072 | Polimento, balanceamento e docs | Arte ANSI do chefe `forest_necromancer` (sem ela, `renderBossArt` lançava erro e o combate do boss quebraria no jogo real). `regionTheme`/`regionBand` já têm fallback para `dark_woods`. Docs atualizados: ARCHITECTURE (seção linear vs. grafo de descoberta), GDD e CONTENT_BIBLE (Bosque Sombrio). `docs/spec.md` e `docs/PLANO_FASE16_MUNDO.md` removidos (temporários — a pedido), com as referências ajustadas. Status da FASE 16 → concluída; painel `backlog.html` atualizado. `yarn check`/`tsc`/`test:run` (328) /`build` verdes |
| T074 | Buscar inimigo (grind sem viajar) | Nova ação **`hunt`** no menu de exploração em grafo (`exploreGraph`): `huntEnemy` sorteia um inimigo do `enemyPool` da região, escala ao nível do jogador (`scaledEnemy`, ver D-10) e resolve um combate avulso — não conclui locais e dispensa o vai-e-volta entre POIs. O menu (`isGraph`) ganhou "Buscar inimigo"; i18n `explore.hunt`/`world.hunting`/`world.huntEmpty` (en/ptBR). 1 teste novo (caça sem viajar). 335 testes |
---
...
- **D-10 — Escala leve de inimigos + grind no grafo:** no grafo de descoberta (FASE 16) o jogador chegava ao chefe sub-nivelado e ficava **travado** — cada local de combate só podia ser vencido uma vez, sem fonte de XP. Duas mudanças (abordagem híbrida do `PROGRESSAO.md`, alavanca barata): (1) locais de **combate** voltaram a ser **repetíveis** ao revisitar (lore/boss continuam de uma vez); (2) `core/scaling.ts` (`scaleEnemy`, puro) escala stats e recompensas do inimigo ao nível do jogador, limitado a `region.maxLevel` via `effectiveEnemyLevel` — só para **cima** (nunca trivializa). Com `maxLevel = 6`, o Necromante permanece nível 6 (marco fixo) enquanto os inimigos comuns acompanham o jogador, mantendo o grind eficiente e o desafio relevante. Aplicado nos call-sites de combate do grafo (`resolveLocationContent`), preservando a curva de XP/recompensa existente.
...
- **Ajustes de polimento pós-FASE 16:** (1) **Diálogo** — opções já escolhidas ficam **acinzentadas**; `NpcState.spokenOptions` persiste as chaves faladas e `runDialogue` as registra. (2) **Borda do painel desalinhada** — `visibleLength` (panel) contava o seletor de variação U+FE0F (de `🛣️`/`🏚️`) como coluna, desalinhando a moldura nessas linhas; passou a ignorar U+FE00–FE0F (corrige todos os painéis). (3) **Mapa sempre à vista** — em modo grafo a tela de exploração desenha o mapa da região + legenda no lugar do texto "Você percorre a região…".
