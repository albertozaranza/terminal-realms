# CONTENT_BIBLE.md

# Terminal Realms Content Bible

Este documento define o conteúdo oficial do jogo.

Toda criatura, item, habilidade, região e chefe deve seguir estas definições.

---

# Filosofia de Conteúdo

O mundo deve transmitir:

- Fantasia medieval clássica
- Progressão constante
- Descoberta
- Perigo crescente
- Recompensas significativas

Evitar:

- Humor excessivo
- Referências modernas
- Tecnologia avançada
- Conteúdo sci-fi

---

# Classes

## Guerreiro

Especialização:

- Sobrevivência
- Defesa
- Combate corpo a corpo

### Escalamento

```text
HP          Muito Alto
Mana        Baixa
Força       Alta
Destreza    Média
Inteligência Baixa
```

---

## Arqueiro

Especialização:

- Críticos
- Velocidade
- Dano consistente

### Escalamento

```text
HP          Médio
Mana        Média
Força       Média
Destreza    Muito Alta
Inteligência Média
```

---

## Mago

Especialização:

- Dano explosivo
- Controle
- Magias

### Escalamento

```text
HP          Baixo
Mana        Muito Alta
Força       Baixa
Destreza    Média
Inteligência Muito Alta
```

---

# Regiões

---

## Região 1

Campos Iniciais

### Níveis

```text
1 - 5
```

### Temática

- Fazendas
- Ruínas
- Florestas leves

### Inimigos

- Rato Gigante
- Goblin
- Lobo
- Bandido

### Chefe

Rei Goblin

---

## Região 2

Floresta Sombria

### Níveis

```text
5 - 15
```

### Temática

- Névoa
- Árvores antigas
- Corrupção

### Inimigos

- Aranha Gigante
- Esqueleto
- Xamã Goblin
- Corvo Sombrio

### Chefe

Necromante da Floresta

---

## Região 3

Montanhas Congeladas

### Níveis

```text
15 - 25
```

### Temática

- Neve
- Tempestades
- Fortalezas abandonadas

### Inimigos

- Lobo do Gelo
- Yeti
- Guerreiro Congelado
- Troll da Montanha

### Chefe

Gigante de Gelo

---

## Região 4

Pântano Maldito

### Níveis

```text
25 - 35
```

### Temática

- Veneno
- Doenças
- Mortos-vivos

### Inimigos

- Zumbi
- Bruxa do Pântano
- Serpente Venenosa
- Espírito Corrompido

### Chefe

Rainha das Bruxas

---

## Região 5

Terras Infernais

### Níveis

```text
35 - 50
```

### Temática

- Lava
- Demônios
- Ruínas infernais

### Inimigos

- Diabrete
- Demônio Guerreiro
- Succubus
- Infernal Guard

### Chefe

Senhor Demoníaco

---

# Chefes

---

## Rei Goblin

### Nível

```text
5
```

### Habilidades

- Golpe Brutal
- Grito de Guerra

### Loot Garantido

- Coroa Goblin
- Ouro

---

## Necromante da Floresta

### Nível

```text
15
```

### Habilidades

- Invocar Esqueletos
- Dreno de Vida

### Loot Garantido

- Cajado Sombrio

---

## Gigante de Gelo

### Nível

```text
25
```

### Habilidades

- Pisotear
- Tempestade Congelante

### Loot Garantido

- Martelo Congelado

---

## Rainha das Bruxas

### Nível

```text
35
```

### Habilidades

- Maldição
- Nuvem Tóxica

### Loot Garantido

- Amuleto Profano

---

## Senhor Demoníaco

### Nível

```text
50
```

### Habilidades

- Chama Infernal
- Explosão Demoníaca
- Forma Verdadeira

### Loot Garantido

- Espada do Abismo

---

# Inimigos Básicos

---

## Rato Gigante

```text
HP: 15
ATK: 3
DEF: 1
XP: 10
```

---

## Goblin

```text
HP: 25
ATK: 5
DEF: 2
XP: 20
```

---

## Lobo

```text
HP: 30
ATK: 6
DEF: 2
XP: 25
```

---

## Bandido

```text
HP: 40
ATK: 8
DEF: 3
XP: 35
```

---

# Slots de Equipamento

```text
weapon
helmet
chest
gloves
boots
ring
amulet
```

---

# Raridades

## Common

Cor ANSI:

```text
white
```

Multiplicador:

```text
1.0x
```

---

## Uncommon

Cor ANSI:

```text
green
```

Multiplicador:

```text
1.2x
```

---

## Rare

Cor ANSI:

```text
blue
```

Multiplicador:

```text
1.5x
```

---

## Epic

Cor ANSI:

```text
magenta
```

Multiplicador:

```text
2.0x
```

---

## Legendary

Cor ANSI:

```text
yellow
```

Multiplicador:

```text
3.0x
```

---

# Armas Iniciais

---

## Guerreiro

Espada Enferrujada

```text
ATK +2
```

---

## Arqueiro

Arco Simples

```text
ATK +2
```

---

## Mago

Cajado de Carvalho

```text
INT +2
```

---

# Habilidades

---

## Guerreiro

### Golpe Poderoso

```text
150% do dano
Cooldown 3
```

### Postura Defensiva

```text
+50% defesa
2 turnos
```

---

## Arqueiro

### Disparo Preciso

```text
200% dano
+20% crítico
```

### Chuva de Flechas

```text
Dano em área
```

---

## Mago

### Bola de Fogo

```text
Dano mágico em área
```

### Raio Arcano

```text
Dano mágico elevado
```

### Escudo Arcano

```text
Redução de dano
```

---

# Consumíveis

---

## Poção Pequena

```text
Recupera 25 HP
```

---

## Poção Média

```text
Recupera 75 HP
```

---

## Poção Grande

```text
Recupera 150 HP
```

---

## Poção de Mana

```text
Recupera 50 Mana
```

---

# Economia

Moeda oficial:

```text
Gold
```

---

# Valores Base

Inimigos:

```text
5 - 100 Gold
```

Chefes:

```text
100 - 1000 Gold
```

Itens comuns:

```text
10 - 50 Gold
```

Itens raros:

```text
100 - 500 Gold
```

Itens lendários:

```text
1000+ Gold
```

---

# Regras de Balanceamento

Todo novo conteúdo deve respeitar:

- Progressão gradual
- Recompensa proporcional ao risco
- Chefes significativamente mais fortes
- Equipamentos nunca devem quebrar a curva de poder

---

# Fonte Única da Verdade

Se existir conflito entre:

- código
- GDD
- documentação

O conteúdo definido neste arquivo deve prevalecer para:

- monstros
- chefes
- regiões
- habilidades
- equipamentos
- loot
- economia

---

# FASE 16 — Bosque Sombrio (grafo de descoberta)

Primeira região explorada como grafo de POIs (data-driven em
`content/regions/darkWoods.ts`):

- **Locais:** Vila Oakheart (entrada), Estrada Principal (goblin), Cabana do
  Caçador (NPC), Banca do Mercador (loja), Coração do Bosque (lobo), Ruínas
  Antigas (lore → "entrada da cripta"), Cripta Esquecida 🔒 (esqueleto),
  Necromante da Floresta 👑 (chefe, Lv6).
- **NPC:** O Caçador — concede conhecimento e inicia a investigação.
- **Conhecimentos:** goblins na estrada, necromante avistado, cripta antiga,
  entrada da cripta (gate do chefe).
- **Missão:** "Sombras sobre o Bosque" (investigação, objetivos por
  conhecimento/local, não "mate N").
